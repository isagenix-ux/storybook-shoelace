...
const WithIcons = {
  ...
  render: args => {
    const attributes = [`variant="${args.variant}"`, `size="${args.size}"`, args.disabled ? 'disabled' : '', args.loading ? 'loading' : '', args.outline ? 'outline' : '', args.pill ? 'pill' : '', `type="${args.type}"`, args.href ? `href="${args.href}"` : '', args.target ? `target="${args.target}"` : '', args.rel ? `rel="${args.rel}"` : ''].filter(Boolean).join(' ');
    return `
...
//# sourceMappingURL=stories-ShoelaceButton-stories.17e758d349418fcf077a.hot-update.js.map